"""
Project GOAT v0.6 — Stage F Anti-Cherry-Picking Unit Tests
"""
...
def test_stage_f_cannot_filter_subsets_or_mutate_hypothesis():
    """MANDATORY TEST: Favorable subset selection or post-hoc context filtering is prohibited."""
    validator = StageFValidator()

    edge = CandidateEdge(
        proposition_name="Anti-Cherry-Picking Edge F",
    ...
    assert not hasattr(res, "best_context_subset")
    assert not hasattr(res, "selected_symbols")
