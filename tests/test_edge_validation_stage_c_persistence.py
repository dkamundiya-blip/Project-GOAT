"""
Project GOAT v0.6 — Stage C Persistence Integration Unit Tests
"""
...
def test_stage_c_evidence_persistence_and_replay():
    repo = SQLiteEdgeRepository(":memory:")
    validator = StageCValidator()

    edge = CandidateEdge(
        proposition_name="Persistence Edge C",
    ...
    folds = [(rng.normal(0.4, 1.0, 100), rng.normal(0.0, 1.0, 100)) for _ in range(5)]
